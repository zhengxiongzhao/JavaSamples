...
public class DeleteFileUtil {
	...
	public static boolean delete(String fileName){
		...
		if (!file.exists()){
			System.out.println("删除文件失败：" + fileName + "(Msg:文件不存在！)");
			return false;
		} else {
			if (file.isFile()){
	...
	public static boolean deleteFile(String fileName){
		...
		if (file.exists() && file.isFile()){
			if(file.delete()){
				System.out.println("删除单个文件" + fileName + "(Msg:成功！)");
				return true;
			} else {
				System.out.println("删除单个文件" + fileName + "(Msg:失败！)");
				return false;
			}
		}else{
			System.out.println("删除单个文件失败：" + fileName + "(Msg:文件不存在！)");
			return false;
		}
	}
	...
	public static boolean deleteDirectory(String dir){
		...
		if (!dirFile.exists() || (!dirFile.isDirectory())){
			System.out.println("删除目录失败：" + dir + "（Msg:目录不存在！）");
			return false;
		 }
		boolean flag = true;
		//删除文件夹下所有文件（包括子目录）
		...
		if (!flag){
			System.out.println("Msg:删除目录失败！");
			return false;
		}
		//删除当前目录
		if (dirFile.delete()){
			System.out.println("删除目录" + dir + "（Msg:成功）！");
			return true;
		} else {
			return false;
		}
